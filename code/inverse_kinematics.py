...
class Robot_Manipulator_Controller:
    ...
    def __init__(self):
        ...
        self.radius=4.5
        self.circle_orient=np.array([[1,0,0],[0,0,1],[0,-1,0]])
        self.warning=False
    ...
    def publish(self):
     ...
     self.joint_angpos = np.radians(self.starting_point)


     R06 = R.from_dcm(self.circle_orient)
     quat_d = R06.as_quat()
     T06_d=np.array([self.translation_d])
     h_d=quat_d[0]
     epsilon_d=quat_d[1:4]
     ...
     while not rospy.is_shutdown():
            ...
            ep=np.transpose(T06_d)-np.transpose(T06)
            eo=np.array([h*epsilon_d-h_d*epsilon-np.matmul(self.skew_from_vector(epsilon_d),epsilon)])
            ep = np.squeeze(ep)
            e = np.concatenate((ep.flatten(), np.transpose(eo).flatten()))
            invJ = inv(J)

            e = np.array(e).reshape(-1, 1)
            print(e)

            self.joint_angvel=invJ@self.k@e
            self.Li(self.joint_angpos)
            if(np.linalg.norm(e)<0.7 or self.warning==True):
